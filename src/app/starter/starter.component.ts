...
import { LocationService, SeoService } from '../shared/services';
...
export class StarterComponent implements OnInit {
  ...
  changeDistance(e) {
    ...
    navigator.geolocation.getCurrentPosition(location => {
      ...
      this.locationService.getIPAddress().subscribe((res: any) => {
        this.locationService.getIpLocation(res).then(res => {
          ...
          loc = {
            "lat": res.data.data.location.ll[0],
            "lng": res.data.data.location.ll[1]
          }
    ...
    // }, error => {
    //   this.toasty.error('Please allow browser location to search with geo location')      
    // })
