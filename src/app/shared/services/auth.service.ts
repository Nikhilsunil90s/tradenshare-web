...
export class AuthService {
  ...
  public profileCompleted = new Subject<boolean>(); 
  ...
  getUser(): Promise<any> {
    return this._getUser = this.restangular.one('users', 'me').get().toPromise()
      .then((resp) => {
        this.currentUser = resp.data;
        this.userLoaded.next(resp.data);
  ...
  getOtp(data:any ):Promise<any> {
    return this.restangular.all('auth/getOtp').post(data).toPromise();
  } 
  

  getOtpIfUserExists(data:any ):Promise<any> {
    return this.restangular.all('users/get-otp-for-shop-exists').post(data).toPromise();
  }

  getOtpIfUserNotExists(data:any ):Promise<any> {
    return this.restangular.all('users/get-otp-for-shop-not-exists').post(data).toPromise();
  } 

  checkOtpForShop(data:any ):Promise<any> {
    return this.restangular.all('users/check-otp-for-shop').post(data).toPromise();
  } 
  ...
  loginWithOtp(credentials: any): Promise<any> {
    return this.restangular.all('auth/loginWithOtp').post(credentials).toPromise()
      .then((resp) => {
  ...
  checkOtp(data: any): Promise<any> {
    return this.restangular.all('users/checkOtp').post(data).toPromise();

  }

  generateTokenForSellerLogin(credentials: any): Promise<any> {
    return this.restangular.all('users/generate-token').post(credentials).toPromise();
  }

  checkUserExists(credentials: any) : Promise<any> {
    return this.restangular.all('auth/checkUserExists').post(credentials).toPromise();
  }
  ...
  autologin(credentials: any): Promise<any> {
    return this.restangular.all('auth/autologin').post(credentials).toPromise()
      .then((resp) => {
        localStorage.setItem('accessToken', resp.data.token);
        return this.restangular.one('users', 'me').get().toPromise()
          .then(resp => {
  ...
  registerSeller(credentials: any): Promise<any> {
    return this.restangular.all('shops/register').post(credentials).toPromise();
  }

  verifyEmail(data): Promise<any> {
    return this.restangular.all('auth/emailVerify').post(data).toPromise();
  } 

  verifyEmailToken(data): Promise<any> {
    return this.restangular.all('auth/verifyEmailToken').post(data).toPromise();
  }
  ...
  createShop(id:string, type:any): Promise<any> {
    return this.restangular.all('users/createShop').customPOST({'_id':id, 'type':type}).toPromise();
  }

  getUserByToken(token:string) {
    return this.restangular.all('auth/autologin').customPOST({'token': token }).toPromise().then((resp) => {
  ...
  removeLoginToken(token:string) {
    return this.restangular.one('user/token', token).customDELETE().toPromise();
  }
